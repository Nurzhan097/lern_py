...
def exchange():
	...
	try:
		...
		messagebox.showwarning("write decimal", "Введите число!")
...
# Header 
h_currency = Label(header_frame, text="Валюта", bg="#444", font="Arial 12 bold")
h_currency.grid(row=0, column=0, sticky=EW)

h_buy = Label(header_frame, text="Покупка", bg="#444", font="Arial 12 bold")
h_buy.grid(row=0, column=1, sticky=EW)

h_sale = Label(header_frame, text="Продажа", bg="#444", font="Arial 12 bold")
...
# btn
btn_uah = ttk.Button(calc_frame, text="Обмен", command=exchange)
btn_uah.grid(row=0, column=2, columnspan=2, sticky=EW, padx=10)
...
l_rub.grid(row=2, column=0)
e_rub = ttk.Entry(res_frame, justify=CENTER, font="Arial 10")
e_rub.grid(row=2, column=1, columnspan=2, padx=10, sticky=EW)
